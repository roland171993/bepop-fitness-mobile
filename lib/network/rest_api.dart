...
Future<FitBotListResponse> getFitBotList() async {
  return FitBotListResponse.fromJson(await handleResponse(
      await buildHttpResponse('chatgpt-fit-bot-list', method: HttpMethod.GET)));
}
...
Future<SocialLoginResponse> saveScore(Map req) async {
  return SocialLoginResponse.fromJson(await handleResponse(
      await buildHttpResponse('save-score',
          request: req, method: HttpMethod.POST)));
}

Future<GameResponse> getGamerRecord({int? page = 1}) async {
  return GameResponse.fromJson(await (handleResponse(await buildHttpResponse(
      "get-score?page=$page",
      method: HttpMethod.GET))));
}

Future<PostList> getPostsApi({int? page = 1}) async {
  return PostList.fromJson(await handleResponse(await buildHttpResponse(
      'userpost-list?page=$page',
      method: HttpMethod.GET)));
}

Future<PostDetailModel> getPostDetailApi({required int postId}) async {
  return PostDetailModel.fromJson(await handleResponse(await buildHttpResponse(
      'userpost-detail?id=${postId}',
      method: HttpMethod.GET)));
}

Future<FitnessBaseResponse> reportApi(Map req) async {
  return FitnessBaseResponse.fromJson(await handleResponse(
      await buildHttpResponse('report-on-posting',
          request: req, method: HttpMethod.POST)));
}

Future<FitnessBaseResponse> saveCommentApi(Map req) async {
  return FitnessBaseResponse.fromJson(await handleResponse(
      await buildHttpResponse('save-comment',
          request: req, method: HttpMethod.POST)));
}

Future<PostList> updateCommentApi(Map req, int? id) async {
  return PostList.fromJson(await handleResponse(await buildHttpResponse(
      'comment-update/$id',
      request: req,
      method: HttpMethod.POST)));
...
Future<FitnessBaseResponse> likePostApi(Map req) async {
  return FitnessBaseResponse.fromJson(await handleResponse(
      await buildHttpResponse('like-userpost',
          request: req, method: HttpMethod.POST)));
}

Future<FitnessBaseResponse> updateReCommentApi(Map req) async {
  return FitnessBaseResponse.fromJson(await handleResponse(
      await buildHttpResponse('update-comment',
          request: req, method: HttpMethod.POST)));
}

Future<PostList> deletePostMediaApi(Map req) async {
  return PostList.fromJson(await handleResponse(await buildHttpResponse(
...
Future<PostList> deletePostApi(Map req) async {
  return PostList.fromJson(await handleResponse(await buildHttpResponse(
      'delete-userpost',
      request: req,
      method: HttpMethod.POST)));
...
Future<FitnessBaseResponse> bookMarkPostApi(Map req) async {
  return FitnessBaseResponse.fromJson(await handleResponse(
      await buildHttpResponse('bookmark-userpost',
          request: req, method: HttpMethod.POST)));
}

Future<BookmarkPostModel> getBookMarkPostsApi({int? page = 1}) async {
  return BookmarkPostModel.fromJson(await handleResponse(
      await buildHttpResponse('my-bookmark-post-list?page=$page',
          method: HttpMethod.GET)));
}

Future<FitnessBaseResponse> saveReCommentApi(Map req) async {
  return FitnessBaseResponse.fromJson(await handleResponse(
      await buildHttpResponse('save-comment-reply',
          request: req, method: HttpMethod.POST)));
}

Future<FitnessBaseResponse> deleteReCommentApi(Map req) async {
  return FitnessBaseResponse.fromJson(await handleResponse(
      await buildHttpResponse('delete-comment',
          request: req, method: HttpMethod.POST)));
}

Future<FitnessBaseResponse> deleteCommentReplyApi(Map req) async {
  return FitnessBaseResponse.fromJson(await handleResponse(
