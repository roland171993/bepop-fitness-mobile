...
class PaymentScreenState extends State<PaymentScreen> {
  ...
  Future<void> paymentListApiCall() async {
    ...
    await getPaymentApi().then((value) {
      appStore.setLoading(false);
      print("---------127>>>${value.toJson()}");
      paymentList.addAll(value.data!);
      if (paymentList.isNotEmpty) {
        paymentList.forEach((element) {
          if (element.type == PAYMENT_TYPE_STRIPE) {
            print('----------132>>>${element.testValue?.secretKey}');
            print('----------133>>>${element.liveValue?.publishableKey}');
...
/* void payStackPayment(BuildContext context) async {
      ...
      'purchase_units': [
        {
          'amount': {
            'currency_code': 'USD',
            'value': widget.mSubscriptionModel?.price.toString() ?? "0.00",
          },
  ...
  Future<void> flutterWaveCheckout() async {
    ...
    var body = {
      ...
      "amount": widget.mSubscriptionModel?.price,
  ...
  void orangeMoneyPayment() async {
    ...
    await BambaraView(
      data: BambaraData(
        ...
        email: userStore.email.validate(),
        name: userStore.username.validate(),
        //publicKey: "lJ4DAUhW38ufbPQ2896AguqOUfxHNyuIjdrGiR2uNUUN",
  ...
  /* void paytmPayment() async {
    ...
    var url = 'https://securegw-stage.paytm.in/theia/api/v1/initiateTransaction?mid=${paytmMerchantId}&orderId=${orderId}';
    print("---------854>>${paytmMerchantId}");
    print("---------853>>${paytmMerchantKey}");
    print("---------852>>${orderId}");
    var body = json.encode({
      "mid": paytmMerchantId,
      "key_secret": paytmMerchantKey,
    ...
    try {
      ...
      String txnToken = response.body;
      print("--------------917>>${paytmMerchantId ?? ''}");
      print("--------------918>>${orderId ?? ''}");
      print("--------------919>>${widget.mSubscriptionModel!.price.toString()}");
      print("--------------912>>${txnToken}");
      print("--------------916>>${isPaytmTestType}");
